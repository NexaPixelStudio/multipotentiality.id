...
export default function FeedbackBox({ feedback, exercise, separatorMode, isCorrect, onNext }) {
  ...
  return (
    <section className={`rounded-[1.75rem] border p-5 shadow-soft ${feedback.correct ? 'border-coach-green bg-coach-sky/25 dark:border-coach-sky/40 dark:bg-emerald-400/8' : 'border-rose-200 bg-rose-50 dark:border-rose-400/25 dark:bg-rose-400/8'}`}>
      <p className={`text-xs font-black uppercase tracking-[0.18em] ${feedback.correct ? 'text-coach-green dark:text-coach-sky' : 'text-rose-700 dark:text-rose-200'}`}>
      ...
      {isCorrect && exercise && (
        <div className="mt-5 space-y-4">
          <div className="rounded-2xl bg-white p-4 dark:bg-black/20">
            ...
            <code className="mt-2 block break-words rounded-xl bg-coach-beige p-3 font-mono text-sm font-black text-coach-green dark:bg-white/8 dark:text-coach-sky">
          ...
          <div className="rounded-2xl bg-white/70 p-4 text-sm leading-6 text-black/65 dark:bg-black/20 dark:text-white/65">
            <span className="font-black text-coach-green dark:text-coach-sky">Contoh penerapan lain: </span>
            {exercise.nextUseCase}
          </div>

          <button onClick={onNext} className="rounded-full bg-coach-ink px-5 py-3 text-sm font-black text-white transition hover:bg-coach-clay dark:bg-white dark:text-coach-ink">
