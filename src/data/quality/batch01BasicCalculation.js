...
const numberTable = {
  title: 'Data Performa Bulanan',
  description: 'Kolom B berisi Penjualan, kolom C berisi Qty Order, kolom D berisi Biaya Operasional, dan kolom E berisi Rating. Pakai tabel ini untuk latihan SUM, AVERAGE, MIN, MAX, LARGE, dan SMALL.',
...
const mixedTable = {
  title: 'Data Campuran: Angka, Teks, dan Kosong',
  description: 'Tabel ini sengaja berisi angka, teks, angka 0, dan cell kosong. Pakai untuk membedakan COUNT, COUNTA, dan COUNTBLANK.',
...
const salesTable = {
  ...
  description: 'Kolom B adalah Kategori, kolom C adalah Kota, kolom D adalah Qty, kolom E adalah Total Penjualan, dan kolom F adalah Rating. Pakai tabel ini untuk latihan rumus dengan kriteria.',
...
const nRangeHints = (goal, range, extra = '') => [
  ...
  `Hint 2: Pilih ${range} karena range itu berisi angka yang diminta soal.`,
...
const oneCriteriaHints = ({ goal, criteriaRange, criteria, valueRange, functionName }) => {
  const base = [
    ...
    `Hint 2: Pilih ${criteriaRange} sebagai range kriteria, karena range ini berisi data tempat Excel mencari ${criteria}.`,
    `Hint 3: Masukkan kriteria ${criteria} persis seperti yang tertulis di tabel atau soal.`
  ];
  if (valueRange) base.push(`Hint 4: Pilih ${valueRange} sebagai range angka yang akan dihitung setelah barisnya lolos syarat.`);
...
const twoCriteriaHints = ({ goal, resultRange, criteriaRange1, criteria1, criteriaRange2, criteria2, functionName, countOnly = false }) => {
  ...
  if (!countOnly) hints.push(`Hint 2: Pilih ${resultRange} sebagai range angka hasil yang mau dihitung.`);
  hints.push(`Hint ${countOnly ? 2 : 3}: Syarat pertama: pilih ${criteriaRange1}, lalu masukkan ${criteria1}.`);
  hints.push(`Hint ${countOnly ? 3 : 4}: Syarat kedua: pilih ${criteriaRange2}, lalu masukkan ${criteria2}.`);
...
const exercises = {
  sum: [
    ...
    { title: 'Januari sampai Juni', question: 'Berapa total penjualan dari Januari sampai Juni?', logic: 'Soal ini meminta total dari bulan Januari sampai Juni saja. SUM tetap sama, tapi range-nya dibatasi ke B2:B7.', expectedFormula: '=SUM(B2:B7)', hints: nRangeHints('total penjualan Januari sampai Juni', 'B2:B7', 'Hint 4: Jangan pilih sampai Desember, karena soal hanya meminta Januari sampai Juni.') },
    { title: 'Juli sampai Desember', question: 'Berapa total penjualan dari Juli sampai Desember?', logic: 'Soal meminta total dari bulan Juli sampai Desember saja. Pilih range penjualan mulai B8 sampai B13.', expectedFormula: '=SUM(B8:B13)', hints: nRangeHints('total penjualan Juli sampai Desember', 'B8:B13', 'Hint 4: Pastikan range dimulai dari Juli, bukan Januari.') },
    { title: 'Tantangan', question: 'Berapa total gabungan antara seluruh penjualan dan seluruh biaya operasional?', logic: 'SUM bisa menerima lebih dari satu range. Di soal ini, jumlahkan range Penjualan dan range Biaya Operasional dalam satu rumus.', expectedFormula: '=SUM(B2:B13,D2:D13)', hints: ['Hint 1: Soalnya meminta dua kelompok angka sekaligus: Penjualan dan Biaya Operasional.', 'Hint 2: Pilih B2:B13 untuk seluruh penjualan.', 'Hint 3: Setelah separator, pilih D2:D13 untuk seluruh biaya operasional.', 'Hint 4: SUM boleh memakai lebih dari satu range jika keduanya sama-sama ingin dijumlahkan.', 'Hint 5: Cek separator antara dua range dan kurung penutupnya.'] }
  ],
  average: [
    ...
    { title: 'Januari sampai Juni', question: 'Berapa rata-rata penjualan dari Januari sampai Juni?', logic: 'Rata-rata hanya untuk bulan Januari sampai Juni, jadi range tidak boleh sampai Desember.', expectedFormula: '=AVERAGE(B2:B7)', hints: nRangeHints('rata-rata penjualan Januari sampai Juni', 'B2:B7') },
    { title: 'Juli sampai Desember', question: 'Berapa rata-rata penjualan dari Juli sampai Desember?', logic: 'Pilih range Juli sampai Desember saja, lalu gunakan AVERAGE.', expectedFormula: '=AVERAGE(B8:B13)', hints: nRangeHints('rata-rata penjualan Juli sampai Desember', 'B8:B13') }
  ],
  min: [
    ...
    { title: 'Januari sampai Juni', question: 'Berapa penjualan paling kecil dari Januari sampai Juni?', logic: 'Range dibatasi Januari sampai Juni agar MIN hanya membaca data bulan yang diminta.', expectedFormula: '=MIN(B2:B7)', hints: nRangeHints('penjualan paling kecil Januari sampai Juni', 'B2:B7') },
    { title: 'Juli sampai Desember', question: 'Berapa penjualan paling kecil dari Juli sampai Desember?', logic: 'Range dibatasi Juli sampai Desember agar MIN hanya membaca data bulan yang diminta.', expectedFormula: '=MIN(B8:B13)', hints: nRangeHints('penjualan paling kecil Juli sampai Desember', 'B8:B13') }
  ],
  max: [
    ...
    { title: 'Januari sampai Juni', question: 'Berapa penjualan paling besar dari Januari sampai Juni?', logic: 'Range dibatasi Januari sampai Juni agar MAX hanya membaca data bulan yang diminta.', expectedFormula: '=MAX(B2:B7)', hints: nRangeHints('penjualan paling besar Januari sampai Juni', 'B2:B7') },
    { title: 'Juli sampai Desember', question: 'Berapa penjualan paling besar dari Juli sampai Desember?', logic: 'Range dibatasi Juli sampai Desember agar MAX hanya membaca data bulan yang diminta.', expectedFormula: '=MAX(B8:B13)', hints: nRangeHints('penjualan paling besar Juli sampai Desember', 'B8:B13') }
  ],
  large: [
    { title: 'Dasar', question: 'Berapa penjualan terbesar ke-2 dari seluruh bulan?', logic: 'LARGE mencari angka terbesar berdasarkan urutan. Angka k menentukan peringkat terbesar yang dicari.', expectedFormula: '=LARGE(B2:B13,2)', hints: ['Hint 1: Soalnya mencari penjualan terbesar ke-2, bukan nilai terbesar pertama.', 'Hint 2: Pilih B2:B13 sebagai range penjualan.', 'Hint 3: Masukkan 2 sebagai k karena yang diminta peringkat terbesar ke-2.', 'Hint 4: Urutan LARGE adalah array dulu, lalu k.', 'Hint 5: Jangan memakai MAX, karena MAX hanya mencari terbesar pertama.'] },
    { title: 'Qty', question: 'Berapa Qty Order terbesar ke-3?', logic: 'Gunakan LARGE pada kolom Qty Order dan isi k dengan 3.', expectedFormula: '=LARGE(C2:C13,3)', hints: ['Hint 1: Soal mencari Qty Order terbesar ke-3.', 'Hint 2: Pilih C2:C13 sebagai range Qty Order.', 'Hint 3: Masukkan 3 sebagai k.', 'Hint 4: LARGE membaca k sebagai peringkat dari yang paling besar.', 'Hint 5: Jangan pilih kolom Penjualan karena yang ditanya Qty Order.'] },
    { title: 'Biaya', question: 'Berapa biaya operasional terbesar ke-2?', logic: 'Gunakan LARGE pada kolom Biaya Operasional.', expectedFormula: '=LARGE(D2:D13,2)', hints: ['Hint 1: Soal mencari biaya operasional terbesar ke-2.', 'Hint 2: Pilih D2:D13.', 'Hint 3: Masukkan 2 sebagai peringkat terbesar.', 'Hint 4: Urutannya array, lalu k.', 'Hint 5: Gunakan hanya range data biaya operasional yang diminta soal.'] },
    { title: 'Rating', question: 'Berapa rating tertinggi ke-3?', logic: 'Rating tertinggi ke-3 berarti pakai LARGE pada kolom Rating dengan k 3.', expectedFormula: '=LARGE(E2:E13,3)', hints: ['Hint 1: Soal mencari rating tertinggi ke-3.', 'Hint 2: Pilih E2:E13 sebagai range rating.', 'Hint 3: Masukkan 3 sebagai k.', 'Hint 4: LARGE mengurutkan dari terbesar.', 'Hint 5: Pastikan tidak memakai SMALL karena arahnya berbeda.'] },
    { title: 'Januari sampai Juni', question: 'Berapa penjualan terbesar ke-2 dari Januari sampai Juni?', logic: 'Range hanya Januari sampai Juni, lalu k bernilai 2.', expectedFormula: '=LARGE(B2:B7,2)', hints: ['Hint 1: Fokus hanya Januari sampai Juni.', 'Hint 2: Pilih B2:B7.', 'Hint 3: Masukkan 2 karena diminta terbesar ke-2.', 'Hint 4: Jangan pilih B2:B13 karena itu seluruh tahun.', 'Hint 5: Cek urutan array dan k.'] },
    { title: 'Tantangan', question: 'Berapa penjualan terbesar ke-4 dari seluruh bulan?', logic: 'Gunakan range seluruh penjualan dengan k 4.', expectedFormula: '=LARGE(B2:B13,4)', hints: ['Hint 1: Soal mencari peringkat terbesar ke-4.', 'Hint 2: Pilih B2:B13.', 'Hint 3: Masukkan 4 sebagai k.', 'Hint 4: LARGE cocok untuk mencari peringkat dari angka terbesar.', 'Hint 5: Kalau memakai MAX, hasilnya hanya terbesar pertama.'] }
  ],
  small: [
    { title: 'Dasar', question: 'Berapa penjualan terkecil ke-2 dari seluruh bulan?', logic: 'SMALL mencari angka terkecil berdasarkan urutan. Angka k menentukan peringkat terkecil yang dicari.', expectedFormula: '=SMALL(B2:B13,2)', hints: ['Hint 1: Soalnya mencari penjualan terkecil ke-2.', 'Hint 2: Pilih B2:B13 sebagai range penjualan.', 'Hint 3: Masukkan 2 sebagai k.', 'Hint 4: SMALL membaca k dari yang paling kecil.', 'Hint 5: Jangan memakai MIN karena MIN hanya mencari terkecil pertama.'] },
    { title: 'Qty', question: 'Berapa Qty Order terkecil ke-3?', logic: 'Gunakan SMALL pada kolom Qty Order dan isi k dengan 3.', expectedFormula: '=SMALL(C2:C13,3)', hints: ['Hint 1: Soal mencari Qty Order terkecil ke-3.', 'Hint 2: Pilih C2:C13.', 'Hint 3: Masukkan 3 sebagai k.', 'Hint 4: SMALL mengurutkan dari terkecil.', 'Hint 5: Jangan pilih kolom Penjualan karena yang ditanya Qty Order.'] },
    { title: 'Biaya', question: 'Berapa biaya operasional terkecil ke-2?', logic: 'Gunakan SMALL pada kolom Biaya Operasional.', expectedFormula: '=SMALL(D2:D13,2)', hints: ['Hint 1: Soal mencari biaya operasional terkecil ke-2.', 'Hint 2: Pilih D2:D13.', 'Hint 3: Masukkan 2 sebagai k.', 'Hint 4: Urutannya array, lalu k.', 'Hint 5: Gunakan hanya range data yang diminta soal.'] },
    { title: 'Rating', question: 'Berapa rating terendah ke-3?', logic: 'Rating terendah ke-3 berarti pakai SMALL pada kolom Rating dengan k 3.', expectedFormula: '=SMALL(E2:E13,3)', hints: ['Hint 1: Soal mencari rating terendah ke-3.', 'Hint 2: Pilih E2:E13.', 'Hint 3: Masukkan 3 sebagai k.', 'Hint 4: SMALL mengurutkan dari terkecil.', 'Hint 5: Pastikan tidak memakai LARGE.'] },
    { title: 'Januari sampai Juni', question: 'Berapa penjualan terkecil ke-2 dari Januari sampai Juni?', logic: 'Range hanya Januari sampai Juni, lalu k bernilai 2.', expectedFormula: '=SMALL(B2:B7,2)', hints: ['Hint 1: Fokus hanya Januari sampai Juni.', 'Hint 2: Pilih B2:B7.', 'Hint 3: Masukkan 2 karena diminta terkecil ke-2.', 'Hint 4: Jangan pilih seluruh tahun.', 'Hint 5: Cek urutan array dan k.'] },
    { title: 'Tantangan', question: 'Berapa penjualan terkecil ke-4 dari seluruh bulan?', logic: 'Gunakan range seluruh penjualan dengan k 4.', expectedFormula: '=SMALL(B2:B13,4)', hints: ['Hint 1: Soal mencari peringkat terkecil ke-4.', 'Hint 2: Pilih B2:B13.', 'Hint 3: Masukkan 4 sebagai k.', 'Hint 4: SMALL cocok untuk mencari peringkat dari angka terkecil.', 'Hint 5: Kalau memakai MIN, hasilnya hanya terkecil pertama.'] }
  ],
  count: [
    ...
    { title: 'Area Lebih Luas', question: 'Berapa banyak cell angka pada seluruh area A2:D13?', logic: 'Area A2:D13 berisi nama, angka, catatan, status, dan cell kosong. COUNT tetap hanya menghitung cell yang benar-benar angka.', expectedFormula: '=COUNT(A2:D13)', hints: ['Hint 1: Soalnya meminta jumlah cell angka, bukan jumlah semua cell yang terisi.', 'Hint 2: Pilih area A2:D13 karena area itu yang diminta soal.', 'Hint 3: COUNT akan mengabaikan teks seperti Order A, Selesai, OK, Pending, dan Cek.', 'Hint 4: Cell kosong juga tidak ikut dihitung.', 'Hint 5: Setelah range masuk, tutup kurung dan tekan Enter.'] },
    { title: 'Kolom Catatan', question: 'Berapa banyak cell angka pada kolom Catatan C2:C13?', logic: 'Kolom Catatan sengaja berisi teks dan cell kosong. COUNT dipakai untuk membuktikan bahwa teks seperti Selesai, Pending, dan Retur tidak dihitung sebagai angka.', expectedFormula: '=COUNT(C2:C13)', hints: ['Hint 1: Soalnya tetap mencari cell angka, walaupun kolom Catatan terlihat terisi teks.', 'Hint 2: Pilih C2:C13 karena itu range Catatan yang ditanya.', 'Hint 3: COUNT tidak menghitung teks seperti Selesai, Pending, atau Retur.', 'Hint 4: Cell kosong juga tidak ikut dihitung.', 'Hint 5: Kalau hasilnya 0, itu benar karena tidak ada angka di kolom Catatan.'] },
    { title: 'Dua Kolom', question: 'Berapa banyak cell angka jika kolom Nilai Angka B2:B13 dan Status D2:D13 dihitung sekaligus?', logic: 'COUNT bisa menerima lebih dari satu range. Range Status sengaja ikut dipilih, tetapi karena isinya teks atau kosong, yang terhitung tetap hanya cell angka.', expectedFormula: '=COUNT(B2:B13,D2:D13)', hints: ['Hint 1: Soalnya meminta dua range sekaligus: Nilai Angka dan Status.', 'Hint 2: Pilih B2:B13 untuk kolom Nilai Angka.', 'Hint 3: Setelah separator, pilih D2:D13 untuk kolom Status.', 'Hint 4: COUNT akan mengabaikan teks seperti OK, Cek, dan Pending.', 'Hint 5: Pastikan dua range dipisahkan separator.'] },
    ...
    { title: 'Order G sampai L', question: 'Berapa banyak cell angka pada Nilai Angka dari Order G sampai Order L?', logic: 'Range dibatasi dari Order G sampai Order L.', expectedFormula: '=COUNT(B8:B13)', hints: nRangeHints('jumlah angka dari Order G sampai Order L', 'B8:B13') }
  ],
  counta: [
    ...
    { title: 'Order G sampai L', question: 'Berapa banyak cell terisi dari Order G sampai Order L pada area A8:D13?', logic: 'Range dibatasi pada enam order terakhir.', expectedFormula: '=COUNTA(A8:D13)', hints: nRangeHints('jumlah cell terisi dari Order G sampai Order L', 'A8:D13') }
  ],
  countblank: [
    ...
    { title: 'Order G sampai L', question: 'Berapa banyak cell kosong dari Order G sampai Order L pada area A8:D13?', logic: 'Range dibatasi pada enam order terakhir.', expectedFormula: '=COUNTBLANK(A8:D13)', hints: nRangeHints('jumlah cell kosong dari Order G sampai Order L', 'A8:D13') }
  ],
  countif: [
    ...
    { title: 'Kota', question: 'Berapa jumlah produk yang berada di kota Jakarta?', logic: 'COUNTIF mengecek satu kolom kriteria, lalu menghitung berapa baris yang cocok.', expectedFormula: '=COUNTIF(C2:C13,"Jakarta")', hints: oneCriteriaHints({ goal: 'jumlah produk di Jakarta', criteriaRange: 'C2:C13', criteria: '“Jakarta”', functionName: 'COUNTIF' }) },
    ...
    { title: 'Kota Bandung', question: 'Berapa jumlah produk yang berada di kota Bandung?', logic: 'Fokus pada kolom Kota, lalu hitung baris yang berisi Bandung.', expectedFormula: '=COUNTIF(C2:C13,"Bandung")', hints: oneCriteriaHints({ goal: 'jumlah produk di Bandung', criteriaRange: 'C2:C13', criteria: '“Bandung”', functionName: 'COUNTIF' }) }
  ],
  sumif: [
